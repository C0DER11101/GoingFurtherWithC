...
**The definition of the macro** `assert` **depends on the state of the macro** `NDEBUG` **which is _not defined_ in** `assert.h`. **If** `NDEBUG` **is defined, then** `assert` **is defined as an expression which does nothing. Otherwise** `assert` **will print debugging information if the expression it tests is false**.
